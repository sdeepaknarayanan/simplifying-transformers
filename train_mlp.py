...
from config.train_config import MLPTrainConfig
    ...
    while epoch < conf.epochs + 1:
        ...
        for index, data in enumerate(train_loader):
            ...
            with torch.no_grad():
                ...
                x = base_model.bert.transformer_blocks[config.block].input_sublayer(x, lambda _x: base_model.bert.transformer_blocks[config.block].attention.forward(_x, _x, _x, mask=mask))
                y = base_model.bert.transformer_blocks[config.block].input_sublayer(x, base_model.bert.transformer_blocks[config.block].feed_forward)
