...
def main(conf):
    ...
    while epoch < conf.epochs + 1:
        ...
        for index, data in enumerate(train_loader):
            ...
            # teacher_pred = torch.softmax(teacher_pred, dim=-1)
            # gt = data['bert_label'][torch.arange(data['bert_label'].size(0)), data['mask_index'].long()]

            squished.zero_grad()
            squished.optimizer.zero_grad()

            loss = criterion(y, teacher_pred)
            loss.backward()
            squished.optimizer.step()
