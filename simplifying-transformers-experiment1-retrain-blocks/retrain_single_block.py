...
def train_block(conf):
    ...
    elif config.criterion == 'CosineSimilarity':
        def CosineSimilarity(x,y):
            criterion = torch.nn.CosineSimilarity(2)
            return torch.sum(torch.square(criterion(x,y)))
        criterion = CosineSimilarity
    elif config.criterion == 'aRRMSE':
        def aRRMSE(x,y):
            MSE = torch.nn.MSELoss(reduction='none')
            x = x.flatten(1,2).transpose(0,1)
            y = y.flatten(1,2).transpose(0,1)
            my = torch.mean(y,1).unsqueeze(1).expand(-1, y.shape[1])   
            dif = torch.sum(MSE(y,my),1)
            mult = torch.sum(MSE(y,x),1)
            return torch.mean(torch.sqrt(mult/dif))   
        criterion = aRRMSE
    else:
